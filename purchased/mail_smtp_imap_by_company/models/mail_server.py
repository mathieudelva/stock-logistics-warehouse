    ...
    def send(self, auto_commit=False, raise_exception=False):
        ...
        for mail_server_id, smtp_from, batch_ids in self._split_by_mail_configuration():
            ...
            company_server_id = self.env["ir.mail_server"].search(
                [("default_company", "=", active_company_id.id)], limit=1
            )
            mail_server_id = (
                company_server_id and company_server_id.id or mail_server_id
            )
            # geminatecs
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread_id and hasattr(ModelCtx, "message_update"):
                ...
                thread.sudo().write(
                    {
                        "company_id": alias
                        and alias.alias_domain.company_id.id
                        or related_user.company_id.id
                    }
                )
                # geminatecs
