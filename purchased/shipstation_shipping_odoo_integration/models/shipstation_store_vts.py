...
class shipstation_store_vts(models.Model):
    ...
    shipstation_order_status = fields.Selection(
        [
    ...
    import_shipstation_shipment = fields.Boolean(
        "Is Automatic Import Shipstation Shipment.?",
        default=False,
        copy=False,
        help="If True then auto import tracking number and shipping cost from shipstation.",
    )
    import_shipstation_order = fields.Boolean(
        "Is Automatic Import Shipstation orders.?",
        default=False,
        copy=False,
    ...
    def import_sale_orders_using_cron(self):
        ...
        for store_id in store_ids:
            _logger.info(
                "{} Shipstation Import Order Process Cron Start".format(
                    store_id.store_name
                )
            )
            store_id.sudo().import_order_from_shipstation_using_cron()
            _logger.info(
                "{} Shipstation Import Order Process Cron Finish".format(
                    store_id.store_name
                )
            )
            store_id.sudo().write(
                {"process_msg": " Shipstation Order Import Process Completed."}
            )
            time.sleep(8)
    ...
    def import_order_from_shipstation_with_thred(self):
        ...
        with api.Environment.manage(), db_registry.cursor() as cr:
            api.Environment(cr, SUPERUSER_ID, self._context)
    ...
    def create_product_from_shipstation(self, product_name, product_sku):
        ...
        if product_sku == "null":
            vals.update({"default_code": product_name})
        else:
            vals.update({"default_code": product_sku})
        return vals

    def import_customer_from_shipstation(
        self, configuration, customer_id, operation_id
    ):
        self.ensure_one()
        try:
            ...
            data = "{}:{}".format(api_key, api_secret)
            ...
            try:
                ...
                self.create_shipstation_operation_details(
                    operation_id, response_msg, True, "customer"
                )
                return False
            vals = {}
            _logger.info(
                "Customer Import Response Status Code {}".format(
            ...
            if response_data.status_code == 200:
                ...
                response_msg = "Partner Created : {}".format(partner_obj.name)
                self.create_shipstation_operation_details(
                    operation_id, response_msg, False, "customer"
                )
                self._cr.commit()
                return partner_obj
        except Exception as e:
    ...
    def create_shipstation_operation_details(
        self,
        operation_id,
        shipstation_response_message,
        fault_operaion,
    ...
    def shipstation_order_api_calling_function(
        self, configuration=False, url=False, operation_id=False
    ):
        response_data = False
        data = "{}:{}".format(configuration.api_key, configuration.api_secret)
        encode_data = base64.b64encode(data.encode("utf-8"))
        authrization_data = "Basic {}".format(encode_data.decode("utf-8"))
        headers = {
            "Authorization": authrization_data,
        ...
        try:
            ...
            self.create_shipstation_operation_details(
                operation_id, response_msg, True, "sale_order"
            )
        return response_data

    def shipstation_order_managing_function(
        self,
        configuration=False,
        orders=False,
        operation_id=False,
        product_operation_id=False,
        product_datas=False,
        csvwriter=False,
        customer_csvwriter=False,
        ...
        for order in orders:
            ...
            _logger.info(
                "Date Time Str {} Type : {}".format(date_time_str, type(date_time_str))
            )
            ...
            if sale_order:
                sale_order = sale_order.filtered(
                    lambda so: so.date_order.strftime("%Y-%m-%d") == date
                )
            if not sale_order:
                ...
                if customerId == None:
                    ...
                    if not partner_obj:
                        ...
                        response_msg = "Partner Created : {}".format(partner_obj.name)
                        self.create_shipstation_operation_details(
                            customer_operation_id, response_msg, False, "customer"
                        )
                else:
                    partner_obj = self.env["res.partner"].search(
                        [("shipstation_customerId", "=", customerId)], limit=1
                    )
                if not partner_obj:
                    partner_obj = self.import_customer_from_shipstation(
                        configuration, customerId, customer_operation_id
                    )
                    if not partner_obj:
                        message = "Customer Is Not Found In Order Response! Shipstation Order ID : {}, Shipstation Order Number : {}".format(
                            shipstation_order_id, shipstation_order_number
                        )
                        self.create_shipstation_operation_details(
                            customer_operation_id, order, True, "sale_order"
                        )
                        continue
                ...
                state_id = self.env["res.country.state"].search(
                    [("code", "=", shipping_partner_state)], limit=1
                )
                country_id = self.env["res.country"].search(
                    [("code", "=", shipping_partner_country)], limit=1
                )
                vals = {
                ...
                carrier_id = self.env["delivery.carrier"].search(
                    [
                        ...
                        (
                            "shipstation_delivery_carrier_service_id.service_code",
                            "=",
                            serviceCode,
                        ),
                    ],
                ...
                if not carrier_id:
                    ...
                    if not carrierCode == None and not serviceCode == None:
                        ...
                        ].search([("code", "=", carrierCode)])
                        shipstation_service_id = self.env[
                            "shipstation.delivery.carrier.service"
                        ].search([("service_code", "=", serviceCode)])
                        if shipstation_carrier_id and shipstation_service_id:
                            delivery_package_id = self.env[
                                "shipstation.delivery.package"
                            ].search([("package_code", "=", packageCode)], limit=1)
                            carrier_id = self.env["delivery.carrier"].create(
                                {
                                    ...
                                    "product_id": self.env.ref(
                                        "shipstation_shipping_odoo_integration.shipstation_service_product"
                                    ).id,
                                }
                            )

                shipstation_warehouse_id = order.get("advancedOptions") and order.get(
                    "advancedOptions"
                ).get("warehouseId")
                _logger.info(
                    " Advance Option >>>>>>>>>>>>>>>>>>>>>{}".format(
                        order.get("advancedOptions")
                    )
                )
                shipstation_warehouse_obj = self.env[
                    "shipstation.warehouse.detail"
                ].search([("warehouse_id", "=", shipstation_warehouse_id)], limit=1)
                _logger.info(
                    "Shipstation Warehouse : {} Shipstation Warehouse obj:{}".format(
                        shipstation_warehouse_id, shipstation_warehouse_obj
                    )
                )
                warehouse_id = False
                ...
                if not warehouse_id:
                    response_msg = "Warehouse  is not availabel :{} ".format(
                        shipstation_warehouse_id
                    )
                ...
                customField1 = (
                    order.get("advancedOptions").get("customField1")
                    if order.get("advancedOptions").get("customField1")
                    else ""
                )
                customField2 = (
                    order.get("advancedOptions").get("customField2")
                    if order.get("advancedOptions").get("customField2")
                    else ""
                )
                customField3 = (
                    order.get("advancedOptions").get("customField3")
                    if order.get("advancedOptions").get("customField3")
                    else ""
                )
                customField_deatils = "{} : {} : {}".format(
                ...
                for order_line in order.get("items"):
                    ...
                    if not product_id:
                        # vals
                        product_vals = self.create_product_from_shipstation(
                        ...
                        response_msg = "{} Product Created".format(product_id.name)
                        self.create_shipstation_operation_details(
                            product_operation_id, response_msg, True, "product"
                        )
                    if not product_id:
                        ...
                        if not shipstation_product_data_obj.sudo().search(
                            ...
                            shipstation_product_data_obj.create(
                ...
                if is_create_order:
                    ...
                    response_msg = "Sale Order Created {}".format(order_id.name)
                    _logger.info("Sale Order Created {}".format(order_id.name))
                    self.sudo().create_shipstation_operation_details(
                        operation_id, response_msg, False, "sale_order"
                    )
                    if carrier_id and carrier_id.delivery_type == "shipstation":
                        # rate_record = carrier_id.shipstation_rate_shipment(order=order_id)
                        base_shipping_cost = order.get(
                            "shippingAmount", 0.0
                        )  # rate_record.get('price')
                    ...
                    for order_line in order.get("items"):
                        ...
                        if not product_sku:
                            ...
                            product_id = (
                                ...
                                .sudo()
                                .search([("default_code", "=", product_sku)], limit=1)
                            )
                            _logger.info("ELSE CONDITION : {}".format(product_id))
                        if not product_id:
                            vals = self.create_product_from_shipstation(
                                product_name, product_sku
                            )
                            product_template_id = self.env["product.template"].create(
                                vals
                        ...
                        if price < 0:
                            product_id = self.env.ref(
                                "shipstation_shipping_odoo_integration.shipstation_discount"
                            )
                        vals = {
                        ...
                        order_line = (
                            self.sudo().create_sale_order_line_from_shipstation(vals)
                        )
                        if option_name:
                    ...
                    if order_id.shipstation_store_id.add_vat_line:
                        ...
                        if vat_price > 0:
                            vat_product_id = self.env.ref(
                                "shipstation_shipping_odoo_integration.shipstation_vat"
                            )
                            taxline_vals = {
                                "product_id": vat_product_id.id,
                                "price_unit": vat_price,
                                "product_uom_qty": 1,
                                "order_id": order_id.id,
                                "name": vat_product_id.name,
                                "company_id": self.env.user.company_id.id,
                            }
                    ...
                    message = "Shipstation Order ID : {}, Shipstation Order Number : {}, Order Is Not Created In Odoo Beacuse of Some Order Products Are Not Available In Odoo. Product Information : {}".format(
                        shipstation_order_id,
                        shipstation_order_number,
                        product_error_message,
                    )
                    self.create_shipstation_operation_details(
                        operation_id, message, True, "sale_order"
                    )
                    self._cr.commit()
            else:
                message = "Order Is Already Exist {}".format(
    ...
    def import_order_from_shipstation_using_cron(self):
        ...
        self.env["shipstation.operation.details"]
        ...
        operation_id = shipstation_operation_detail.create(
            {
                ...
                "shipstation_operation_type": "import",
                "message": "Sale Order Import Process Running...",
                "shipstation_store_id": self.id,
        ...
        customer_operation_id = shipstation_operation_detail.create(
            {
                "shipstation_operation": "customer",
                "shipstation_operation_type": "import",
                "message": "Customer Import Process Running...",
                "shipstation_store_id": self.id,
                "import_order_from_date": self.last_modification_date,
                "import_order_to_date": self.shipstation_to_date,
            }
        ...
        try:
            ...
            last_modification_date = "{}T{}.0000000".format(
                last_modify_date, last_modify_time
            )
            _logger.info(
                "Last Modification Date >>>>>> {} Today Date : {}".format(
                    last_modification_date, shipstation_to_date
                )
            )
            url = configuration.making_shipstation_url(
                "/orders?orderDateStart={}&orderDateEnd={}&storeId={}&orderStatus={}&pageSize=500&sortBy={}".format(
            ...
            response_data = self.shipstation_order_api_calling_function(
                configuration, url, operation_id
            )
            _logger.info(
                ">>>>> Response Data Status {} >> {}".format(
                    response_data.status_code, response_data.json()
                )
            )
            product_datas = []
            headers = []
            ...
            if response_data.status_code == 200:
                ...
                if total_pages >= 1:
                    for total_page in range(1, total_pages + 1, 1):
                        if total_page <= 0:
                            continue
                        page_url = (
                            "{}&orderStatus={}&pageSize=500&page={}&sortBy={}".format(
                                url,
                                self.shipstation_order_status,
                                total_page,
                                "OrderDate",
                        ...
                        response_data = self.shipstation_order_api_calling_function(
                            configuration, page_url, operation_id
                        )
                        if response_data.status_code == 200:
                            responses = response_data.json()
                            orders = responses.get("orders")
                            if not orders:
                                response_msg = "In Shipstation Import Order process, Not found Orders!  {}".format(
                                    responses
                                )
                                self.create_shipstation_operation_details(
                                    operation_id, response_msg, True, "sale_order"
                            ...
                            if isinstance(orders, dict):
                                orders = [orders]
                            product_datas = self.shipstation_order_managing_function(
                                configuration,
                                orders,
                                operation_id,
                            ...
                            if response_data.json():
                                error_detail = {
                                    ...
                                    + " - "
                                    + error_message
                                    + " - %s" % (response_data.json())
                                }
                            response_msg = (
                                "1 >>>>>>> Getting an Issue in the  Imported  Process %s"
                                % (error_detail)
                            )
                            self.create_shipstation_operation_details(
                                operation_id, response_msg, True, "sale_order"
                    ...
                    if create_csv_file:
                        ...
                        files_path = "/tmp/product_Details_{}/".format(
                            fields.Date.today()
                        )
                        ...
                        self.env["ir.attachment"].create(
                            {
                                "datas": datas,
                                "name": "product_Details_{}.csv".format(
                                    fields.Date.today()
                                ),
                    ...
                    customer_csvwriter = DictWriter(
                        customer_buffer, customer_headers, delimiter=","
                    )
                    ...
                    customer_csv_file = StringIO(
                        base64.decodestring(customer_file_datas).decode()
                    )
                    customer_files_path = "/tmp/Customer_Details_{}/".format(
                        fields.Date.today()
                    )
                    ...
                    customer_files_path = "{}Customer_Details.csv".format(
                        customer_files_path
                    )
                    ...
                    attachment = self.env["ir.attachment"].create(
                        {
                            ...
                            "name": "Customer_Details_{}.csv".format(
                                fields.Date.today()
                            ),
                            "res_model": "shipstation.operation.detail",
                ...
                response_msg = (
                    "2 >>>>>>>> Getting an Issue in the  Imported  Process %s"
                    % (error_detail)
                )
                self.create_shipstation_operation_details(
                    operation_id, response_msg, True, "sale_order"
            ...
            response_msg = (
                "3 >>>>>>>>>> Getting an Issue in the  Imported  Process %s" % (e)
            )
            self.process_msg = " Process InCompleted Please Check Log"
            _logger.info(response_msg)
            self.create_shipstation_operation_details(
        ...
        shipstation_product_data_obj = self.env["shipstation.product.data"]
        shipstation_product_data_ids = shipstation_product_data_obj.search(
            [("store_id", "=", self.id)]
        )
        ...
        shipstation_process_ids = shipstation_operation_data_ids.filtered(
            lambda op: len(op.operation_ids) <= 0
        )
    ...
    def import_order_from_shipstation(self):
        with api.Environment.manage():
            ...
            try:
                configuration = self.env["shipstation.odoo.configuration.vts"].search(
                    [], limit=1
                )  # 'id', '=', self.shipstation_configuration_id.id
                if not configuration:
                    self.create_shipstation_operation_details(
                        operation_id,
                        "Shipstation Configuration Missing",
                ...
                url = configuration.making_shipstation_url(
                    "/orders?orderDateStart={}&orderDateEnd={}&storeId={}&orderStatus={}&pageSize=500".format(
                        last_modification_date,
                        shipstation_to_date,
                        self.store_id,
                        self.shipstation_order_status,
                ...
                if response_data.status_code == 200:
                    ...
                    if total_pages >= 1:
                        for total_page in range(1, total_pages + 1, 1):
                            if total_page <= 0:
                                continue
                            page_url = "{}&orderStatus={}&pageSize=500&page={}&sortBy={}".format(
                                url,
                                self.shipstation_order_status,
                                total_page,
                                "OrderDate",
                            ...
                            if response_data.status_code == 200:
                                ...
                                if isinstance(orders, dict):
                                    orders = [orders]
                                product_datas = (
                                    self.shipstation_order_managing_function(
                                        configuration,
                                        orders,
                                        operation_id,
                                        product_operation_id,
                                ...
                                self.process_msg = (
                                    " Process InCompleted Please Check Log"
                                )
                                _logger.info(response_msg)
                            self._cr.commit()
            ...
            self.last_modification_date = from_order_date
            _logger.info(
                "Last Modification Date  >>>> {}".format(self.last_modification_date)
            )
            to_order_date = datetime.now() + relativedelta(hours=1)
            _logger.info("Current Date >>>> {}".format(to_order_date))
            to_order_date = to_order_date.strftime("%Y-%m-%d")  # %H:%M:%S
            self.shipstation_to_date = "{} 23:59:59".format(to_order_date)
            _logger.info("To Order Date  >>>> {}".format(to_order_date))
            operation_id.message = "Order Import Process Completed."
            customer_operation_id.message = "Customer Import Process Completed."
            product_operation_id.message = "Product Import Process Completed."
    ...
    def action_view_shipstation_message_detail(self):
        action = self.env.ref(
            "shipstation_shipping_odoo_integration.action_shipstation_operation_detail"
        ).read()[0]
