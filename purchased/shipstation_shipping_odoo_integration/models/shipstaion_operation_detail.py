...
class ShipstationOperationDetail(models.Model):
    ...
    shipstation_operation = fields.Selection(
        [
            ("store", "store"),
    ...
    shipstation_operation_type = fields.Selection(
        [
            ("export", "Export"),
            ("import", "Import"),
            ("update", "Update"),
        ...
        string="Shipstation Operation Type",
    )
    warehouse_id = fields.Many2one("stock.warehouse", "Warehouse")
    company_id = fields.Many2one("res.company", "Company")
    operation_ids = fields.One2many(
        "shipstation.operation.details", "operation_id", string="Operation"
    )
    message = fields.Char("Message")
    shipstation_store_id = fields.Many2one(
        "shipstation.store.vts", string="Shipstation Store"
    )
    import_order_from_date = fields.Datetime(
        string="Shipstation Import Order From Date"
    )
    ...
    def create(self, vals):
        sequence = self.env.ref(
            "shipstation_shipping_odoo_integration.seq_shipstation_operation_detail"
        )
...
class ShipstationOperationDetails(models.Model):
    ...
    shipstation_operation = fields.Selection(
        [
            ...
            ("shipment", "Shipment"),
            ("sale_order", "Sale Order"),
            ("delivery_order", "Shipment Orders"),
            ("product", "Product"),
            ("warehouse", "Warehouse"),
            ("customer", "Customer"),
        ],
    ...
    warehouse_id = fields.Many2one(
        "stock.warehouse", "Warehouse", related="operation_id.warehouse_id"
    )
    company_id = fields.Many2one("res.company", "Company")
    ...
    def create(self, vals):
        if type(vals) == dict:
            ...
            operation = (
                operation_id
                and self.env["shipstation.operation.detail"].browse(operation_id)
                or False
            )
            company_id = (
                operation and operation.company_id.id or self.env.user.company_id.id
            )
