...
class SaleOrder(models.Model):
    ...
    shipstation_shipping_charge_id = fields.Many2one(
        "shipstation.shipping.charge",
        string="Shipstation Service",
    ...
    shipstation_order_number = fields.Char(
    ...
    shipstation_store_id = fields.Many2one(
        "shipstation.store.vts", string="Shipstation Store", copy=False
    )
    ...
    is_exported_to_shipstation = fields.Boolean(
        string="Order Exported to Shipstation", default=False, copy=False
    )

    def auto_process_shipstation_orders(self):
        order_ids = self.env["sale.order"].search(
            [("state", "=", "draft")], order="id", limit=500
        )
        for order in order_ids:
            ...
            for pick in order.picking_ids:
                ...
                pick.action_done()
                _logger.info("Done Order >>>>> :{}".format(order))
...
class SaleOrderLine(models.Model):
    ...
    def compute_package_volume(self):
        for line in self:
            line.product_volume = (
                line.product_id.width
                * line.product_id.height
