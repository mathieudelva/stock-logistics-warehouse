...
class StockPicking(models.Model):
    ...
    is_order_imported_in_shipstation = fields.Boolean(
        "Imported Shipstation Shipment",
        copy=False,
        default=False,
        help="If Checked, This order is Imported From shipstation.",
    )
    shipstation_service_code = fields.Char(
        string="Shipstation Shipment Wise Service Code", copy=False
    )
    ...
    shipstation_sale_order_number = fields.Char(
        string="Shipstation Order Number", copy=False
    )
    ...
    def update_order_in_shipstation(self):
        ...
        try:
            ...
            response_data = self.carrier_id and self.carrier_id.api_calling_function(
    ...
    def generate_label_from_shipstation(self):
        ...
        for package_id in self.package_ids:
            ...
            try:
                ...
                carrier_id = (
                    package_id.carrier_id if package_id.carrier_id else self.carrier_id
                )
                ...
                if response_data.status_code == 200:
                    ...
                    if shipment_id:
                        ...
                        tracking_message = _("Shipstation Tracking Number: </b>%s") % (
                            tracking_number
                        )
                        ...
                        self.message_post(
                            body=logmessage,
                            attachments=[("%s.pdf" % (self.id), label_data_pdf)],
                        )
                else:
                    error_code = "%s" % (response_data.status_code)
                    ...
                    error_detail = {
                        ...
                        + " - "
                        + error_message
                        + " - %s" % (response_data.text or response_data.content)
                    }
                    # if response_data.json():
        ...
        if self.weight_bulk:
            try:
                ...
                body = (
                    self.carrier_id
                    and self.carrier_id.generate_label_from_shipstation(
                        self, False, weight
                    )
                )
                ...
                if response_data.status_code == 200:
                    ...
                    if shipment_id:
                        ...
                        base_data = binascii.a2b_base64(str(label_data))
                        label_datas.append(base_data)
                        tracking_mesage = _("Shipstation Tracking Number: </b>%s") % (
                            tracking_number
                        )
                        message_id = self.message_post(
    ...
    def _prepare_picking_for_immediate_transfer(self):
        ...
        if self.state == "done":
            _logger.info(
                "%s (Backend Process) : Could not process. Picking already in DONE state."
                % self.name
            )
            return False
        if self.state == "draft":
            ...
            if self.state != "assigned":
                ...
                if self.state != "assigned":
                    _logger.info(
                        "%s (Backend Process) : Could not reserve all requested products. Please use the 'Check Availability' button to handle the reservation manually."
                        % self.name
                    )
                    return False
        ...
        if self.state == "assigned":
            ...
            if self._check_backorder():
                _logger.info(
                    "%s (Backend Process) : Could not transfer because there is a possibility of BackOrder. Please check delivery order."
                    % self.name
                )
                for move in self.move_ids:
                    for move_line in move.move_line_ids:
    ...
    def import_delivery_order_using_cron(self):
        ...
        _logger.info(
            "Total PICKING : {} >>>>>>>>>>>>>> Pickings : {}".format(
                len(pickings), pickings
            )
        )
        ...
        operation_id and operation_id.write(
            {
                "message": "Shipstation Shipment Import Process Done Successfully.",
            }
        )
    ...
    def import_delivery_order(self, operation_id=False):
        ...
        try:
            ...
            url = configuration.making_shipstation_url(
                "/shipments?orderNumber=%s" % (shipstation_sale_order_number)
            )
            ...
            try:
                ...
                shipstation_operation_details.create(
                    {
                        "operation_id": operation_id and operation_id.id,
                        "shipstation_response_message": "Shipstation Import Order Issue %s, %s"
                        % (self.name, e),
                        "fault_operaion": True,
                        "shipstation_operation": "shipment",
                    }
            ...
            if response_data.status_code == 200:
                ...
                if not shipments:
                    shipstation_operation_details.create(
                        {
                            "operation_id": operation_id and operation_id.id,
                            "shipstation_response_message": "%s Data is not availabel. %s"
                            % (self.name, responses),
                            "fault_operaion": True,
                            "shipstation_operation": "shipment",
                        }
                ...
                for shipment in shipments:
                    ...
                    if shipment.get("voided") != True:
                        ...
                        shipstation_unit_of_measure = shipment.get("weight").get(
                            "units"
                        )
                ...
                self.shipstation_unit_of_measure = ",".join(
                    shipstation_unit_of_measures
                )
                ...
                self._cr.commit()
                _logger.info(
                    "Shipment Data Imported For This Picking : %s." % (self.name)
                )
            else:
                error_code = "%s" % (response_data.status_code)
                error_message = response_data.reason
                ...
                shipstation_operation_details.sudo().create(
                    {
                        "operation_id": operation_id and operation_id.id,
                        "shipstation_response_message": "%s Delivery order Not Imported %s"
