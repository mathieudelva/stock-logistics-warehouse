...
class ShipstationOdooIntegationConfig(models.Model):
    ...
    api_secret = fields.Char(
        string="API Secret", help="Use Your Shipstation Password as API Secret."
    )
    ...
    def create_shipstation_operation_details(
        self, operation_id, shipstation_response_message, fault_operaion=False
    ):
    ...
    def action_get_shipstation_stores(self):
        action = self.env.ref(
            "shipstation_shipping_odoo_integration.actionid_shipstation_store_vts"
        ).read()[0]
        return action

    def action_action_get_shipstation_providers(self):
        action = self.env.ref(
            "shipstation_shipping_odoo_integration.actionid_shipstation_delivery_carrier_vts"
        ).read()[0]
        return action

    def action_get_shipstation_services(self):
        action = self.env.ref(
            "shipstation_shipping_odoo_integration.actionid_shipstation_delivery_carrier_service_vts"
        ).read()[0]
        return action

    def action_get_shipstation_packages(self):
        action = self.env.ref(
            "shipstation_shipping_odoo_integration.actionid_shipstation_delivery_package_vts"
        ).read()[0]
        return action
    ...
    def delivery_carrier_package_process(self):
        ...
        for carrier in carrier_name:
            url = self.making_shipstation_url(
                "/carriers/listpackages?carrierCode=%s" % (carrier.code)
            )
            ...
            shipstation_operation_details = self.env["shipstation.operation.details"]
            shipstation_delivery_carrier_package = self.env[
            ...
            try:
                ...
                for response in responses:
                    shipstation_delivery_carrier_package = (
                        shipstation_delivery_carrier_package.search(
                            [
                                ("package_code", "=", response.get("code")),
                    ...
                    if not shipstation_delivery_carrier_package:
                        ...
                        shipstation_delivery_carrier_package.create(
                            {
                                "name": response.get("name", False),
                                "package_code": response.get("code", False),
                                "service_nature": "domestic"
                                if response.get("domestic", False)
                                else "international",
                                "delivery_carrier_id": carrier_id.id,
                                "supported_domestic": True
                                if response.get("domestic")
                                else False,
                                "supported_international": True
                                if response.get("international")
                                else False,
                            }
                        )
                        shipstation_operation_details.create(
                ...
                shipstation_operation_details.create(
                    {
                        "operation_id": operation_id.id,
                        "shipstation_response_message": e,
                        "fault_operaion": True,
    ...
    def delivery_carrier_service_process(self):
        carrier_name = self.env["shipstation.delivery.carrier"].search([])
        shipstation_delivery_carrier_service = self.env[
        ...
        for carrier in carrier_name:
            ...
            url = self.making_shipstation_url(
                "/carriers/listservices?carrierCode=%s" % (carrier.code)
            )
            response = self.api_calling_function(url)
            if response.status_code != 200:
                error = "Error Code : %s - %s" % (response.status_code, response.reason)
                ...
                shipstation_operation_details.create(
                    {
                        "operation_id": operation_id.id,
                        "shipstation_response_message": responses.get("message"),
                        "fault_operaion": True,
                        "shipstation_operation": "carrier_service",
                    }
                )
            ...
            try:
                responses = response.json()
                for response in responses:
                    shipstation_delivery_carrier_service = (
                        shipstation_delivery_carrier_service.search(
                            [("name", "=", response.get("name", False))]
                        )
                    )
                    if not shipstation_delivery_carrier_service:
                        ...
                        shipstation_delivery_carrier_service.create(
                            {
                                "name": response.get("name", False),
            ...
            for response in responses:
                ...
                if not delivery_carrier:
                    shipstation_delivery_carrier.create(
                        {
                            ...
                            "shipping_provider_id": response.get(
                                "shippingProviderId", False
                            ),
                        }
                    )
                    shipstation_operation_details.create(
                        {
                            "operation_id": operation_id.id,
                            "shipstation_response_message": "%s Delivery Carrier Created"
                            % (response.get("name")),
                            "fault_operaion": False,
                            "shipstation_operation": "delivery_carrier",
                        }
                    ...
                    shipstation_operation_details.create(
                        {
                            "operation_id": operation_id.id,
                            "shipstation_response_message": "%s Delivery Carrier already exist"
                            % (response.get("name")),
                            "fault_operaion": True,
                            "shipstation_operation": "delivery_carrier",
                        }
        ...
        try:
            ...
            for response in responses:
                store = shipstation_store.search(
                    [("store_id", "=", response.get("storeId", False))]
                )
                if not store:
                    ...
                    shipstation_operation_details.create(
                        {
                            "operation_id": operation_id.id,
                            "shipstation_response_message": "%s Store Created"
                            % (response.get("storeName")),
                            "fault_operaion": False,
                            "shipstation_operation": "store",
                        }
                    )
                    ...
                    shipstation_operation_details.create(
                        {
                            "operation_id": operation_id.id,
                            "shipstation_response_message": "%s Store already exist"
                            % (response.get("storeName")),
                            "fault_operaion": True,
                            "shipstation_operation": "store",
                        }
        ...
        try:
            ...
            if not configuration:
                ...
                self.create_shipstation_operation_details(
                    operation_id, response_msg, True
                )
                return
            ...
            try:
                ...
                response_msg = "Shipstation Import Warehouse Issue %s, %s" % (
                    self.name,
                    e,
                )
                self.create_shipstation_operation_details(
            ...
            if response_data.status_code == 200:
                ...
                for warehouse in responses:
                    ...
                    if not sh_warehouse:
                        ...
                        if warehouse:
                            response_msg = "Warehouse Created Updated : {}".format(
                                warehouse.name
                            )
                            self.create_shipstation_operation_details(
                                operation_id, response_msg, False
                            )
        except Exception as e:
