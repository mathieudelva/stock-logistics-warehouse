...
class StocklocationContentCheckLine(models.Model):
    ...
    check_id = fields.Many2one(
        "stock.location.content.check", string="Check", required=True
    )
    expected_qty = fields.Float(
        compute="_compute_quantities", string="Expected Quantity"
    )
    current_qty = fields.Float(
        compute="_compute_quantities", string="Current Quantity", required=True
    )
    counted_qty = fields.Float(string="Counted Quantity", required=True)
    replenished_qty = fields.Float(string="Replenished Quantity", required=True)
    ...
    def _compute_quantities(self):
        for rec in self:
            ...
            rec.current_qty = sum(quant.inventory_quantity for quant in quants)
